...
Карта не использует скачанные модели. Город рисуется самим приложением на `canvas`: тип здания, его уровень и выбранный участок влияют на внешний вид и экономический результат.
...
| --- | --- |
| Построить жильё | включается выбор участка для доступного и стабильного объекта стоимостью `250 000` |
| Построить офис | включается выбор участка для дорогого объекта с высоким потенциальным доходом стоимостью `400 000` |
| Построить ритейл | включается выбор участка для сбалансированного коммерческого объекта стоимостью `320 000` |
...
После нажатия на кнопку строительства карта переходит в режим размещения. Свободные участки подсвечиваются, а на каждом доступном участке появляются два показателя:

- **поток района** — общая привлекательность места для жителей, клиентов и бизнеса
- **доход** — процентный бонус или штраф именно для выбранного типа здания

При наведении отображаются дополнительные данные: рейтинг участка, эффект для репутации, влияние на общий рынок и причины расчёта.

Выбор зависит от окружения:

- жильё выгоднее строить в жилом районе рядом с парками и променадами
- офисы лучше работают в деловых районах и рядом с другими офисами
- ритейл получает бонус от торговых зон, жилья и делового трафика
- дороги повышают доступность участка
- слишком плотная однотипная застройка усиливает конкуренцию
- парки и променады нельзя застраивать: это общественные зоны, улучшающие соседние участки

После выбора участка здание появляется именно в указанной точке карты. Его локация продолжает влиять на квартальный доход, расходы, цену продажи и развитие рынка.
...
| `plotId` | идентификатор выбранного участка на карте |
| `locationName` | название района размещения |
| `locationDemandAtBuild` | поток района в момент строительства |
| `locationIncomeAtBuild` | коэффициент дохода в момент строительства |
...
Для объекта дополнительно рассчитывается локальный множитель участка:

```text
locationIncomeMultiplier = 1
  + zoneBonus
  + roadAccessBonus
  + publicSpaceBonus
  + nearbyPropertySynergy
  - overDensityPenalty
```

Он зависит от типа района, дорог, парков, променадов, соседних объектов и плотности однотипной застройки. Фактический доход объекта умножается на этот коэффициент, а расходы могут немного увеличиваться в дорогих или перегруженных районах.
...
- `getPlotEvaluation()` рассчитывает поток района, рейтинг и коэффициенты участка
- `beginBuildingPlacement()` включает режим выбора площадки
- `handleCityCanvasClick()` подтверждает строительство на выбранном участке
